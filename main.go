...
func main() {
	...
	flag.StringVar(&linterface_string,"i","","the interface to listen to")
	flag.Parse()
	...
	log.Println("flag.lookup:",reflect.TypeOf(flag.Lookup("u").Value))
	if isUdp {
	...
	if !isListen {
		...
		if linterface_string==""{
			destinationPort = fmt.Sprintf(":%v", flag.Arg(0))
			log.Println("No interface defined Destination port is"+destinationPort)
			log.Println("Interface is:"+linterface_string)
		} else{
			linterface:=net.ParseIP(linterface_string)
			destinationPort = fmt.Sprintf("%s:%v", linterface,flag.Arg(0))
			log.Println("Destination port is:"+destinationPort)
		}
	}
